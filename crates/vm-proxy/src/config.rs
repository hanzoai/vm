...
/// A host port exposed to the guest via host.vm.internal.
#[derive(Debug, Clone)]
pub struct ExposeHostMapping {
    /// Port on the host (127.0.0.1:host_port).
    pub host_port: u16,
    /// Port the guest connects to (host.vm.internal:guest_port).
    pub guest_port: u16,
}
...
pub struct ProxyConfig {
    ...
    /// holding the resolved vm.json, so relative paths in a config behave
    /// the same wherever hanzo-vm is invoked from. None inherits the process cwd.
