...
mod guest {
    ...
    use vm_proto::{VSOCK_PORT, VSOCK_PORT_FORWARD};
    ...
    fn mount_overlay(tag: &str, guest_path: &str) -> Result<(), String> {
        ...
        eprintln!("vm-guest: mounted {} -> {} (overlay)", tag, guest_path);
        Ok(())
    }
    ...
    fn mount_direct(tag: &str, guest_path: &str) -> Result<(), String> {
        ...
        eprintln!("vm-guest: mounted {} -> {} (direct rw)", tag, guest_path);
        Ok(())
    }
    ...
    fn bring_up_interface(sock: i32, name: &[u8]) {
        unsafe {
            ...
            if libc::ioctl(sock, libc::SIOCGIFFLAGS as _, &mut ifr) < 0 {
                eprintln!("vm-guest: failed to get {} flags", display_name);
                return;
            }

            ifr.ifr_ifru.ifru_flags |= libc::IFF_UP as libc::c_short;
            if libc::ioctl(sock, libc::SIOCSIFFLAGS as _, &ifr) < 0 {
                eprintln!("vm-guest: failed to bring up {}", display_name);
            }
        }
    }
    ...
    fn setup_networking() {
        unsafe {
            ...
            if sock < 0 {
                eprintln!("vm-guest: failed to create socket for networking setup");
                return;
            }
            ...
            if !has_eth0 {
                libc::close(sock);
                eprintln!("vm-guest: no network device (sandbox mode)");
                return;
            }
    ...
    fn create_vsock_listener(port: u32) -> i32 {
        unsafe {
            ...
            if fd < 0 {
                panic!(
                    "vm-guest: failed to create vsock socket: {}",
                    std::io::Error::last_os_error()
                );
            }
            ...
            if ret < 0 {
                panic!(
                    "vm-guest: failed to bind vsock on port {}: {}",
                    port,
                    std::io::Error::last_os_error()
                );
            }
            ...
            if ret < 0 {
                panic!(
                    "vm-guest: failed to listen on vsock: {}",
                    std::io::Error::last_os_error()
                );
            }
                    ...
                    for event in events {
                        ...
                        for path in &event.paths {
                            ...
                            let kind = if event.kind.is_remove() {
                                vm_proto::watch_kind::DELETE
                            } else {
                                vm_proto::watch_kind::MODIFY
                            };
                            let evt = WatchEvent {
                                kind,
    ...
    fn handle_forward_connection(fd: i32) {
        ...
        let tcp_stream = match std::net::TcpStream::connect(("127.0.0.1", req.port)) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("vm-guest: forward to port {} failed: {}", req.port, e);
                let resp = ForwardResponse {
                    status: "error".into(),
    ...
    pub fn run() -> ! {
        ...
        loop {
            ...
            eprintln!("vm-guest: accepted vsock connection");
...
fn main() {
    ...
    {
        eprintln!("vm-guest is a Linux-only binary meant to run inside a VM");
        std::process::exit(1);
    }
}
