...
use crate::socket::SocketDeviceConfiguration;
...
impl VirtualMachineConfiguration {
    ...
    pub fn set_socket_devices<T: SocketDeviceConfiguration>(&self, devices: Vec<T>) {
        let devices: Vec<Id<VZSocketDeviceConfiguration, Shared>> =
            devices.iter().map(|d| d.id()).collect();

        let socket_devices = NSArray::from_vec(devices);
        unsafe {
            self.inner.setSocketDevices(&socket_devices);
        }
    }
