...
// Re-exports
pub use vm::{default_data_dir, MountConfig};
pub use vm_proto::{DirEntry, ReadDirResponse, StatResponse, WatchEvent};
pub use vm_proxy::config::{ExposeHostMapping, NetworkConfig, ProxyConfig, SecretConfig};
...
pub struct SandboxConfig {
    ...
    pub ports: Vec<vm_proto::PortMapping>,
    /// Host ports exposed to the guest via host.vm.internal.
...
impl ShellWriter {
    ...
    pub fn send_input(&self, data: &[u8]) -> Result<()> {
        ...
        vm_proto::frame::write_frame(&mut *w, vm_proto::frame::STDIN, data)?;
        w.flush()?;
        Ok(())
    }
    ...
    pub fn resize(&self, rows: u16, cols: u16) -> Result<()> {
        let mut w = self.writer.lock().unwrap();
        let payload = vm_proto::frame::resize_payload(rows, cols);
        vm_proto::frame::write_frame(&mut *w, vm_proto::frame::RESIZE, &payload)?;
        Ok(())
    }
}
...
enum SandboxCmd {
    ...
    Download {
        ...
        progress_tx: std::sync::mpsc::Sender<vm_proto::DownloadProgress>,
        reply: oneshot::Sender<Result<()>>,
    },
    OpenShell {
...
/// Async wrapper around a Hanzo VM VM sandbox.
///
...
impl AsyncSandbox {
    ...
    pub async fn open_shell(
        ...
        let reader_thread = std::thread::Builder::new()
            .name("vm-shell-reader".into())
            .spawn(move || {
                let mut reader = BufReader::new(reader_stream);
                loop {
                    match vm_proto::frame::read_frame(&mut reader) {
                        Ok(Some((vm_proto::frame::STDOUT, payload))) => {
                            if event_tx.send(ShellEvent::Output(payload)).is_err() {
                                break;
                            }
                        }
                        Ok(Some((vm_proto::frame::EXIT, payload))) => {
                            let code = vm_proto::frame::parse_exit_code(&payload).unwrap_or(0);
                            let _ = event_tx.send(ShellEvent::Exit(code));
                            break;
                        }
                        Ok(Some((vm_proto::frame::ERROR, payload))) => {
    ...
    pub async fn download(
        ...
        std::sync::mpsc::Receiver<vm_proto::DownloadProgress>,
    )> {
        let (reply_tx, reply_rx) = oneshot::channel();
    ...
    pub async fn open_watch(&self, path: &str, recursive: bool) -> Result<WatchHandle> {
        ...
        let reader_thread = std::thread::Builder::new()
            .name("vm-watch-reader".into())
            .spawn(move || {
                let mut reader = BufReader::new(stream);
                loop {
                    match vm_proto::frame::read_frame(&mut reader) {
                        Ok(Some((vm_proto::frame::WATCH_EVENT, payload))) => {
...
struct BootedVm {
    ...
    instance_dir: String,
    data_dir: String,
    proxy_handle: Option<vm_proxy::ProxyHandle>,
    fwd_handle: Option<vm::PortForwardHandle>,
    #[cfg(feature = "cas")]
    nbd_handle: Option<vm_store::NbdHandle>,
    default_env: HashMap<String, String>,
}

fn boot_vm(config: SandboxConfig) -> Result<BootedVm> {
    let default_env = config.env.clone();
    let data_dir = config.data_dir.unwrap_or_else(vm::default_data_dir);
    ...
    let nbd_handle = if use_nbd {
        ...
        Some(vm_store::start_cas_nbd_server(
            &source,
            &cas_dir_str,
            &index_path,
            &socket_path,
    ...
    let (vm_fd, proxy_handle) = if config.allow_net {
        let proxy_config = ProxyConfig {
            ...
            expose_host: config.expose_host,
        ...
        let handle = vm_proxy::start(host_fd, proxy_config)?;
    ...
    // Build the VM
    let mut builder = vm::Sandbox::builder()
        .kernel(&kernel_path)
        .rootfs(&work_rootfs)
        .cpus(config.cpus)
...
fn run_vm_loop(
    ...
    proxy_handle: Option<vm_proxy::ProxyHandle>,
    _fwd_handle: Option<vm::PortForwardHandle>,
    #[cfg(feature = "cas")] nbd_handle: Option<vm_store::NbdHandle>,
    default_env: HashMap<String, String>,
) {
    ...
    let mut extra_fwd_handles: Vec<vm::PortForwardHandle> = Vec::new();
