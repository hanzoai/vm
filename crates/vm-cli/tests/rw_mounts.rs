...
//!   HANZO_VM_BIN=target/debug/hanzo-vm cargo test -p hanzo-vm -- --ignored

use std::process::Command;
use tempfile::tempdir;

fn vm_bin() -> String {
    std::env::var("HANZO_VM_BIN")
        .expect("HANZO_VM_BIN not set — point it at a codesigned hanzo-vm binary (e.g. just build)")
}
...
fn run_in_vm(mount_spec: &str, guest_cmd: &str) -> std::process::Output {
    Command::new(vm_bin())
        ...
        .output()
        .expect("failed to spawn hanzo-vm — is HANZO_VM_BIN correct?")
}
