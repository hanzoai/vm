...
pub(crate) struct VmArgs {
    ...
    /// Path to kernel
    #[arg(long, env = "HANZO_VM_KERNEL")]
    pub kernel: Option<String>,

    /// Path to rootfs image
    #[arg(long, env = "HANZO_VM_ROOTFS")]
    pub rootfs: Option<String>,

    /// Path to initramfs (for loading VirtIO modules)
    #[arg(long, env = "HANZO_VM_INITRD")]
    ...
    /// Expose a host port to the guest via host.vm.internal (HOST:GUEST or PORT)
    #[arg(long = "expose-host", value_name = "HOST:GUEST", hide = true)]
    pub expose_host: Vec<String>,

    /// Path to config file (default: ./vm.json)
    #[arg(long)]
...
#[derive(Parser)]
#[command(name = "hanzo-vm", about = "Hanzo microVM", version)]
...
pub(crate) enum Commands {
    ...
    /// Upgrade hanzo-vm to the latest release (CLI + OS image)
    Upgrade,
