...
use vm::Sandbox;
use vm_proto::{frame, WatchEvent};

use crate::boot::{self, PreparedVm};
...
pub(crate) fn run_stdio(prepared: &PreparedVm, launch: &vm_measure::Log) -> Result<i32> {
    ...
    let nbd_handle = boot::start_nbd(prepared)?;
    let nbd_uri = nbd_handle.as_ref().map(|h| h.uri());

    let sandbox = Arc::new(boot::build_sandbox(
        prepared,
        false,
        vm_fd,
        nbd_uri.as_deref(),
            ...
            method::SPAWN => {
                ...
                std::thread::spawn(move || {
                    ...
                    let stream = match sb.open_exec(&argv, &spawn_env, params.cwd.as_deref()) {
                        Ok(s) => s,
                        Err(e) => {
                            eprintln!("hanzo-vm: spawn failed: {}", e);
            ...
            method::WATCH => {
                ...
                std::thread::spawn(move || {
                    let stream = match sb.open_watch(&path, recursive) {
                        Ok(s) => s,
                        Err(e) => {
                            eprintln!("hanzo-vm: watch failed: {}", e);
                            return;
                        }
                    };
...
fn handle_checkpoint(
    ...
    if let Err(msg) = vm::validate_checkpoint_name(name) {
    ...
    if let Err(e) = boot::clone_file(&prepared.work_rootfs, &checkpoint_path) {
        return send_error_shared(
            out,
            id,
            SERVER_ERROR,
