...
impl SecretEntry {
    fn to_secret_config(&self, name: &str) -> Result<vm_proxy::config::SecretConfig> {
        ...
        match (&self.from, &self.command) {
            ...
            (Some(from), None) => {
                ...
                Ok(vm_proxy::config::SecretConfig::from_env(
                    from.clone(),
                    self.hosts.clone(),
                ))
            }
            (None, Some(command)) => {
                ...
                Ok(vm_proxy::config::SecretConfig {
                    ttl,
                    ..vm_proxy::config::SecretConfig::from_command(
                        command.clone(),
                        self.hosts.clone(),
                    )
                })
...
impl Config {
    /// Reject a malformed config up front, so `hanzo-vm run` fails the same way
    ...
    /// Convert config sections into a ProxyConfig for vm-proxy.
    pub fn to_proxy_config(&self) -> Result<vm_proxy::config::ProxyConfig> {
        let mut proxy = vm_proxy::config::ProxyConfig {
...
pub(crate) fn parse_expose_host(s: &str) -> Result<vm_proxy::config::ExposeHostMapping> {
    ...
    match parts.len() {
        1 => {
            ...
            Ok(vm_proxy::config::ExposeHostMapping {
                host_port: port,
                guest_port: port,
            })
        }
...
pub(crate) fn load_config(config_flag: Option<&str>) -> Result<Config> {
    let path = match config_flag {
        Some(p) => std::path::PathBuf::from(p),
        None => std::path::PathBuf::from("vm.json"),
    };
    ...
    match std::fs::read_to_string(&path) {
        Ok(contents) => {
            ...
            // hanzo-vm happened to be invoked from.
            cfg.config_dir = path
                .canonicalize()
                .ok()
...
mod tests {
    ...
    fn config_from(json: &str) -> Result<Config> {
        let cfg: Config = serde_json::from_str(json)?;
        cfg.validate()?;
        Ok(cfg)
    }

    /// Config has no Debug impl, so unwrap_err is unavailable.
