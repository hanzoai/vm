...
const GITHUB_REPO: &str = "hanzoai/vm";
...
fn download_os_image_version(data_dir: &str, version: &str) -> Result<()> {
    ...
    eprintln!("hanzo-vm: downloading OS image ({})...", tag);
    eprintln!("hanzo-vm: {}", url);
...
fn cli_tarball_name(version: &str) -> Result<String> {
    ...
    Ok(format!("hanzo-vm-v{}-{}.tar.gz", version, platform))
}
...
pub fn upgrade(data_dir: &str) -> Result<()> {
    if let Ok(exe) = std::env::current_exe() {
        if exe.to_string_lossy().contains("/.cargo/bin/") {
            bail!("This copy was installed with cargo. Run `cargo install hanzo-vm` instead");
        }
    }

    eprintln!("hanzo-vm: checking for updates...");
    ...
    if latest == CURRENT_VERSION {
        eprintln!("hanzo-vm: already on latest version ({})", CURRENT_VERSION);
        return Ok(());
    }

    eprintln!("hanzo-vm: upgrading {} -> {}", CURRENT_VERSION, latest);
    ...
    eprintln!("hanzo-vm: downloading CLI ({})...", latest);
    eprintln!("hanzo-vm: {}", cli_url);
    ...
    for entry in archive.entries().context("failed to read CLI archive")? {
        ...
        if entry.path()?.to_str() == Some("hanzo-vm") {
    ...
    if !tmp_path.exists() {
        bail!("'hanzo-vm' binary not found in CLI archive");
    }
    ...
    eprintln!("hanzo-vm: CLI updated to {}", latest);

    // Update OS image
    download_os_image_version(data_dir, latest)?;

    eprintln!("hanzo-vm: upgrade complete ({})", latest);
    Ok(())
}
    ...
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        if current_mb != self.last_printed_mb {
            ...
            if let Some(total) = self.total_bytes {
                ...
                let _ = write!(
                    stderr,
                    "\rhanzo-vm: downloaded {} / {} MB",
                    current_mb, total_mb
                );
            } else {
                let _ = write!(stderr, "\rhanzo-vm: downloaded {} MB", current_mb);
            }
            let _ = stderr.flush();
        }
