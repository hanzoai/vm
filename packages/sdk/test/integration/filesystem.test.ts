/**
 * Integration tests for filesystem operations against the real hanzo-vm VM.
 *
 * Prerequisites:
 *   1. cargo build -p vm-guest --target aarch64-unknown-linux-musl --release
 *   2. ./scripts/prepare-rootfs.sh
 *   3. cargo build -p hanzo-vm
 *   4. codesign --entitlements vm.entitlements --force -s - target/debug/hanzo-vm
 *
...
const HANZO_VM_BIN = resolve(REPO_ROOT, "target/debug/hanzo-vm");

const canRun = existsSync(HANZO_VM_BIN);

describe.if(canRun)("filesystem operations", () => {
	let sb: Sandbox;

	beforeAll(async () => {
		sb = await Sandbox.start({ bin: HANZO_VM_BIN });
	}, 60_000);
...
describe.if(!canRun)("filesystem operations (skipped)", () => {
	test("hanzo-vm binary not found", () => {
		console.log(
			"Build with: cargo build -p hanzo-vm && codesign --entitlements vm.entitlements --force -s - target/debug/hanzo-vm",
		);
	});
});
