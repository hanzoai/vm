...
export class VmProcess {
	...
	async start(args: string[]): Promise<void> {
		...
		await new Promise<void>((resolve, reject) => {
			const timeout = setTimeout(() => {
				reject(new Error("hanzo-vm: timed out waiting for ready signal (30s)"));
			}, 30_000);
	...
	send(
		...
		if (!this.proc) throw new Error("hanzo-vm process not started");
	...
	async stop(): Promise<void> {
		...
		const timeout = new Promise<never>((_, reject) =>
			setTimeout(
				() => reject(new Error("hanzo-vm: shutdown timed out (5s)")),
				5_000,
			),
		);
		...
		for (const [, req] of this.pending) {
			req.reject(new Error("hanzo-vm process stopped"));
		}
		this.pending.clear();
		this.proc = null;
	}
	...
	private async readLoop(): Promise<void> {
		...
		if (this.onReadyError) {
			this.onReadyError(new Error("hanzo-vm process exited unexpectedly"));
			this.onReady = null;
			this.onReadyError = null;
		}
		for (const [, req] of this.pending) {
			req.reject(new Error("hanzo-vm process exited unexpectedly"));
		}
		this.pending.clear();
	}
