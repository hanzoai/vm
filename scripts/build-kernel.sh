...
DEFCONFIG="${REPO_DIR}/kernel/vm_defconfig"
KERNEL_URL="https://cdn.kernel.org/pub/linux/kernel/v${KERNEL_MAJOR}.x/linux-${KERNEL_VERSION}.tar.xz"
BUILD_DIR="${DATA_DIR}/kernel-build"

echo "==> Building custom kernel ${KERNEL_VERSION} for Hanzo VM"
...
if [ "$(uname -m)" = "aarch64" ] && [ "$(uname -s)" = "Linux" ]; then
    ...
    cp "$DEFCONFIG" arch/arm64/configs/vm_defconfig
    make ARCH=arm64 vm_defconfig
    ...
    docker run --rm \
        ...
        -v "${DEFCONFIG}:/tmp/vm_defconfig:ro" \
        ...
        debian:trixie-slim /bin/sh -c '
            ...
            cp /tmp/vm_defconfig arch/arm64/configs/vm_defconfig
            make ARCH=arm64 vm_defconfig > /dev/null 2>&1
